...
const deployVestingPPN = async (manager, ppnAddress, periodStartTime, unlockQuantity) => {
  const VestingPPN = await hre.ethers.getContractFactory("VestingPPN");
  const vestingPPN = await VestingPPN.deploy(
    manager, 
    ppnAddress, 
    periodStartTime, 
    unlockQuantity
  );
  await vestingPPN.deployed();
  return vestingPPN;
};
