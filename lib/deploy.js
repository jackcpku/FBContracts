...
const deployDividend = async (pvsAddress, tokenAddress, amount) => {
  const Dividend = await hre.ethers.getContractFactory("Dividend");
  const dividend = await Dividend.deploy(pvsAddress, tokenAddress, amount);
  await dividend.deployed();
  return dividend;
};

const deployAutoDividend = async (pvsAddress, tokenAddress, startTime, periods) => {
  const Dividend = await hre.ethers.getContractFactory("AutoDividend");
  const dividend = await Dividend.deploy(pvsAddress, tokenAddress, startTime, periods);
  await dividend.deployed();
  return dividend;
}; 
