...
describe("Test Factory & Gateway Contract", function () {
  let gateway, factory;
  let owner, gatewayManager, newGatewayManager, u2, u3, u4;

  beforeEach("Deploy contracts", async function () {
    // Reset test environment.
    await hre.network.provider.send("hardhat_reset");

    [owner, gatewayManager, newGatewayManager, u2, u3, u4] = await hre.ethers.getSigners();
  ...
  describe("Access control", function () {
    ...
    it("Gateway manager should be able to set a new gateway of U2-contract only through gateway contract", async function () {
      ...
      await expect(gateway.connect(gatewayManager).setGatewayOf(u2Contract.address, gateway.address))
        .to.be.revertedWith("Should assign a new gateway");

      // Success: through gateway contract
      await gateway.connect(gatewayManager).setGatewayOf(u2Contract.address, u2.address);
    });

    it("Gateway manager should be able to transfer gateway ownership, others not", async function () {
      // Fail: through u2
      expect(gateway.connect(u2).transferGatewayOwnership(u2.address)).to.be.reverted;

      // Success: through gateway manager
      await gateway.connect(gatewayManager).transferGatewayOwnership(newGatewayManager.address);

      // After the ownership transfer, gatewayManager should not be authorized to 
      // 1. Set factory address
      // 2. Set manager of a certain contract
      // 3. Set gateway of a certain contract
      // 4. Transfer gateway ownership
      let someAddress = u4.address;
      expect(gateway.connect(gatewayManager).setFactoryAddress(someAddress)).to.be.reverted;
      expect(gateway.connect(gatewayManager).setManagerOf(u2Contract.address, someAddress)).to.be.reverted;
      expect(gateway.connect(gatewayManager).setGatewayOf(u2Contract.address, someAddress)).to.be.reverted;
      expect(gateway.connect(gatewayManager).transferGatewayOwnership(someAddress)).to.be.reverted;
